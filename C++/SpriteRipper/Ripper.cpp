...
BOOL Ripper::ClipOutSpriteFrame( unsigned char *FromPtr, unsigned long BoxWidth, unsigned long BoxHeight, unsigned long BufferWidth, int hotSpotX, int hotSpotY )
{
   ...
   if( ! (ImageData = ( unsigned char *) malloc( ( BoxWidth * BoxHeight) * 4 ) ) )
     {
   ...
   for( unsigned long y = 0; y < BoxHeight; y++ )
    ...
    Sprite *newSprite =  new Sprite( ImageData, BoxWidth, BoxHeight, hotSpotX, hotSpotY );     
		
	newSprite ->SetFrameNumber( ++frameNumber );   
	newSprite ->SetTransparentColor( RGB(0,0,255) ); 


    if( ! sprite_list ) // adding root node...  
	  {	 	
        sprite_list = newSprite;
	  }
     else 
	  {	  
       Sprite *workNode = sprite_list;  // start at head node

	   workNode = workNode ->GetLastSprite(); 
  	   workNode ->NextSprite = newSprite;  // attach to list

	   // hook up the last sprite to the list...
       if(workNode)
	      workNode ->NextSprite ->LastSprite = workNode;	   	  
	  }
